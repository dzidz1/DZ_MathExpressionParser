...
typedef enum {
  /* Data Types */
  MATH_CPP_DTYPE_FIRST = (int)MATH_CPP_OPR_INVALID + 1,
  MATH_CPP_INT,
  MATH_CPP_DOUBLE,
  MATH_CPP_STRING,
  MATH_CPP_IPV4,
...
static inline int math_cpp_operator_precedence(int token_code) {
  ...
  assert(!"Unknown operator in math_cpp_operator_precedence");
  return 0;
}

static inline bool math_cpp_is_operator(int opcode) {
  if (opcode < (int)MATH_CPP_OPR_FIRST || opcode >= (int)MATH_CPP_OPR_MAX)
    return false;
  return true;
}

static inline bool math_cpp_is_operand(int opcode) {
  if (opcode < (int)MATH_CPP_DTYPE_FIRST || opcode >= (int)MATH_CPP_DTYPE_LAST)
    return false;
  return true;
}
